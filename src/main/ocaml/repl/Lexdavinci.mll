...
let symbol_table = Hashtbl.create 18
let _ = List.iter (fun (kwd, tok) -> Hashtbl.add symbol_table kwd tok)
                  [("{", SYMB1);("}", SYMB2);("*", SYMB3);("(", SYMB4);(")", SYMB5);("|", SYMB6);("!?", SYMB7);("!!", SYMB8);("=>", SYMB9);("<-", SYMB10);("??", SYMB11);("?!", SYMB12);("@", SYMB13);("'", SYMB14);("[", SYMB15);("]", SYMB16);(";", SYMB17);(",", SYMB18)]

let resword_table = Hashtbl.create 5
let _ = List.iter (fun (kwd, tok) -> Hashtbl.add resword_table kwd tok)
                  [("case", TOK_case);("false", TOK_false);("for", TOK_for);("switch", TOK_switch);("true", TOK_true)]
